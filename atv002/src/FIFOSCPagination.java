...
public class FIFOSCPagination{
    private int[][] ram;
    private MemoryPage mp;

    public FIFOSCPagination(MemoryPage mp){
        this.mp = mp;
        this.ram = mp.getRam();
    }
    ...
    public void execInstructions(){
        ...
        while (executions < 1000) { /*cada algoritmo deve executar 1000 instruções,
            ...
            if(ram[0][1] == rdmNumber){
                ...
                else if(ram[0][3] == 0){ //retirado, e outro é carregado no final via swap
                    if(ram[0][4] == 1){
                        ram[0][4] = 0;
                        swap[ram[0][0]] = ram[0];
                        mp.setSwap(swap);
